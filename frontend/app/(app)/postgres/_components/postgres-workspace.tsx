...
import type { DbInstance, SecurityGroup } from "@/lib/api";
...
export function PostgresWorkspace({
  ...
  initialInstances: DbInstance[];
  initialSecurityGroups: SecurityGroup[];
}) {
  const router = useRouter();
  const [pending, start] = useTransition();
  const [busy, setBusy] = useState<string | null>(null);
  const [name, setName] = useState("app-postgres");
  const [instanceClass, setInstanceClass] = useState("db.t3.small");
  ...
  async function create(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy("create");
    try {
      const res = await fetch("/api/ec2/db-instances", {
        method: "POST",
      ...
      setCreated({
        id: payload.db_instance.db_instance_id,
        username: payload.credentials.username,
        password: payload.credentials.password,
      });
      await refresh();
    } catch (error: any) {
      alert(error?.message ?? "Failed to create Postgres instance");
    } finally {
      setBusy(null);
    }
  }
    ...
    <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
      <Card>
        <CardHeader className="border-b border-border/80">
          <CardTitle className="text-primary">DB instances</CardTitle>
          <CardDescription>
            Each instance is isolated in its own Ubuntu VM.
          </CardDescription>
        </CardHeader>
        <CardContent className="p-0">
          {initialInstances.length === 0 ? (
            <div className="p-10 text-center text-sm text-muted-foreground">
              No Postgres instances have been created.
            </div>
          ) : (
            <Table>
              ...
              <TableBody>
                {initialInstances.map((instance) => (
                  <TableRow key={instance.db_instance_id}>
                    <TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex justify-end gap-2">
                        <Button
                          ...
                          onClick={() => action(`/api/ec2/db-instances/${instance.db_instance_id}/start`, "POST", instance.db_instance_id)}
                        >
                        ...
                        <Button
                          ...
                          size="sm"
                          disabled={working || !instance.vmid}
                          onClick={() => action(`/api/ec2/db-instances/${instance.db_instance_id}/stop`, "POST", instance.db_instance_id)}
                        >
                        ...
                        <Button
                          ...
                          size="sm"
                          disabled={working}
                          onClick={() => action(`/api/ec2/db-instances/${instance.db_instance_id}`, "DELETE", instance.db_instance_id)}
                        >
        ...
        {created ? (
          <Card>
            ...
            <CardContent className="space-y-3 pt-6 text-sm">
              <Credential label="Instance" value={created.id} />
        ...
        <Card>
          ...
          <CardContent className="pt-6">
            <form onSubmit={create} className="space-y-4">
              <Field id="postgres-name" label="Name" value={name} setValue={setName} />
              <div className="space-y-2">
                <Label htmlFor="postgres-class">Class</Label>
                <select
                  id="postgres-class"
                  value={instanceClass}
              ...
              <Field id="postgres-storage" label="Storage GiB" value={storageGiB} setValue={setStorageGiB} type="number" />
              <div className="space-y-2">
                <Label htmlFor="postgres-sg">Security group</Label>
                <select
                  id="postgres-sg"
              ...
              <Button type="submit" disabled={working} className="w-full">
                ...
                <span className="ml-2">Create Postgres</span>
              </Button>
            </form>
          </CardContent>
        </Card>
